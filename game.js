...
function showButton(colour){$("#"+colour).addClass("pressed");setTimeout(function(){$("#"+colour).removeClass("pressed")},200)}
...
return startArray}
function gameOver(){playBtnSound("wrong");$("#level-title").text("Timeout or Wrong Input, Press A Key to Restart");$("body").addClass("game-over");setTimeout(function(){$("body").removeClass("game-over")},200);flag=!1;step=0;level=0}
...
playBtnSound(colourArray[0]);showButton(colourArray[0]);$("#level-title").text("Level "+(level+=1));flag=!0;start=!0;startGameTime=new Date().getTime()}
$(document).keypress(function(event){if(!flag){startGame()}
if(new Date().getTime()-startGameTime>5000){gameOver()}
console.log(event.key);if(flag&&!start){var letter=event.key.toUpperCase();if(letter!==letterArray[step++]){gameOver()}
else if(step<4){playBtnSound(colourArray[step-1]);showButton(colourArray[step-1])}}
if(flag&&step===4){flag=!1;step=0;sleep(1000);startGame()}
start=!1})
